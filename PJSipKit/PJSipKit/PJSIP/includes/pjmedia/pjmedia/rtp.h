...
//add by zhanglei
PJ_DECL(pj_status_t) pjmedia_rtp_encode_rtp_sm2( pjmedia_rtp_session *ses,
					     int pt, int m,
					     int payload_len, int ts_len,
					     const void **rtphdr,
					     int *hdrlen, pjmedia_frame *payload, pj_str_t *pubkey );
//end
...
//add by zhanglei
PJ_DECL(pj_status_t) pjmedia_rtp_decode_rtp_sm2( pjmedia_rtp_session *ses,
					     const void *pkt, int pkt_len,
					     const pjmedia_rtp_hdr **hdr,
					     const void **payload,
					     unsigned *payloadlen, pj_str_t *prikey);

PJ_DECL(pj_status_t) pjmedia_rtp_decode_rtp2_sm2(
				    pjmedia_rtp_session *ses,
				    const void *pkt, int pkt_len,
				    const pjmedia_rtp_hdr **hdr,
				    pjmedia_rtp_dec_hdr *dec_hdr,
